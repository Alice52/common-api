...
public class DesensitizationUtil {
    ...
    /**
     * 正则匹配模式 <br>
     * - 该正则表达式第三个() 可能无法匹配以某些特殊符号开头和结尾的<br>
    ...
    /** 作为 ignoreCase 初始的标记 */
    private Boolean initIgnoreFlag = false;

    /** 作为openFlag初始化的标记 */
    private Boolean initOpenFlag = false;

    /** 所有key:value配置匹配对 */
    public Map<String, Object> allPattern;
    ...
    public String customChange(String eventFormattedMessage) {
        try {
            ...
            boolean flag = false;
            // 获取Yml配置文件内容 - Map格式
            Map<String, Object> patternMap = YmlUtil.patternMap;
            if (!CollectionUtils.isEmpty(patternMap)) {
                // 如果没有开启脱敏，返回""，则不会做脱敏操作
                if (!checkOpen(patternMap)) {
                    return "";
                }
                // 获取一个原始Message的正则匹配器
                ...
                while (regexMatcher.find()) {
                    ...
                    try {
                        ...
                        // 获取Key对应规则
                        Object keyPatternValue = getKeyIgnoreCase(key);
                        if (null != keyPatternValue
                                && null != originalValue
                            ...
                            if (!"null".equals(value) || value.equalsIgnoreCase(key)) {
                                ...
                                String filterData = getBracketPattern(patternVales);
                                ...
                                value =
                                        getReplaceValue(
                                                value, patternVales, split, originalPatternValues);
    ...
    private String getReplaceValue(
            String value, String patternVales, String[] split, String originalPatternValues) {
        if (split.length >= 2 && !"".equals(patternVales)) {
            ...
            value = dataDesensitization(beforeIndexOf, lastIndexOf, value);
        }
        return value;
    }
    ...
    private String getMultiplePattern(Object patternVale, String newValue) {
        if (patternVale instanceof String) {
            ...
            // 获取规则 - Map类型（不推荐，有风险）
            return getPatternByMap((Map<String, Object>) patternVale, newValue);
            ...
            if (patternVale instanceof List) {
                ...
                if (!CollectionUtils.isEmpty(list)) {
                    ...
                    for (; iterator.hasNext(); ) {
                        ...
                        String patternValue = getPatternByMap(map, newValue);
